...
def inject_image_link(md_path: Path, out_path: Path):
    ...
    # Use path relative to the markdown file so GitHub can resolve it when viewing the file
    try:
        img_rel = str(out_path.relative_to(md_path.parent).as_posix())
    except Exception:
        img_rel = out_path.name
